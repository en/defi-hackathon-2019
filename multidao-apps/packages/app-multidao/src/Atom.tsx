...
class Atom extends TxComponent<Props, State> {
  public state: State = {
    assetId: 1,
    assetAmount: 0,
    inherentAmount: 0,
  ...
  async initData () {
    ...
    res = await api.query.multiDao.prices(1)
    let price = Number(res.toString())
    if(price>0){
      price = new Decimal(price).div(10000).toNumber()
    }
    const assetBalance = await this.getBalance(1)
